...
describe("get gists by name thunk", () => {
  it("success", async () => {
    ...
    const dispatch = jest.fn();
    const getPublicGistsApiName = jest.fn().mockResolvedValue({ data: DATA });

    const thunk = getGistsName(NAME);

    await thunk(dispatch, null, { getPublicGistsApiName });

    expect(getPublicGistsApiName).toBeCalledWith(NAME);
    expect(getPublicGistsApiName).toBeCalledTimes(1);

    expect(dispatch).toBeCalledTimes(2);
    expect(dispatch).toHaveBeenNthCalledWith(1, getGistsNameStart());
    expect(dispatch).toHaveBeenNthCalledWith(2, getGistsNameSuccess(DATA));
  });
  ...
  it("error", async () => {
    ...
    const dispatch = jest.fn();
    const getPublicGistsApiName = jest.fn().mockRejectedValue(ERROR);
    ...
    expect(dispatch).toBeCalledTimes(2);
    expect(dispatch).toHaveBeenNthCalledWith(1, getGistsNameStart());
    expect(dispatch).toHaveBeenNthCalledWith(2, getGistsNameError(ERROR));
  });
});
